function [x_target,alpha,knockout] = ratioSelect(model,id_biomass,id_target,TMGR,max_loop,gap,numMultiStrat,glist,k)
...
numglist=numel(glist);
gvalue=zeros(numglist,1);
% init parameters
for gi=1:numglist
    modn1=floor((k-1)/power(2,gi-1));
    gvalue(gi)=rem(modn1,2);
end
alpha=0;
n=size(model.S,2);
...
tex=0;
for gi=1:numglist
    lb(n+nGpr+nAux+glist(gi),1)=gvalue(gi);
    ub(n+nGpr+nAux+glist(gi),1)=gvalue(gi);
end
tStart=tic;
for i=1:max_loop    
    %ratio=ratio+gap;
    ...
    %fname=sprintf("%s/mip_%i",filename,i);
    %[logtxt,cplex,pool]=evalc("CPLEXINTLINPROG(f,A,b,Aeq,beq,lb,ub,ctype)");
    %fid = fopen(fname, 'w');
    %fwrite(fid, logtxt);
    %fclose(fid);
    [cplex,pool]=CPLEXINTLINPROG(f,A,b,Aeq,beq,lb,ub,ctype);
    ...
    if ~isempty(pool)
        ...
        for j=1:size(pool,2)
            ...
            if size(x,1)==n+nGpr+nAux+nGen
                [x_target,knockout_gene,rt]=verifyGeneKnock(model,x(n+nGpr+nAux+1:n+nGpr+nAux+nGen,:),indGPR,id_biomass,id_target,TMGR,0.05);
                tex=tex+rt;
